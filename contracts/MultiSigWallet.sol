...
contract MultiSigWallet{
    ...
    function removeOwner(address removeOwner) Owner public{
        ...
        require(owners[removeOwner] == 1);
        uint i;
        uint replace = 0;
        for(i=0;i<addedOwners.length;i++){
            if(addedOwners[i] == removeOwner){
              replace = 1;
            }
            if(replace == 1){
              addedOwners[i-1] = addedOwners[i];
            }
        }
        delete addedOwners[i-1];
        noOfOwners--;
